...
parser.add_argument(
    '--bpreport', '-bp', type=str, default='standard_report_reduced',
...
parser.add_argument(
    '--subregion', '-r', type = str, default=None,
    help="isolated subregion to highlight the results from")
parser.add_argument(
    '--four', '-f', type = str, default=None,
    help="plot catergories- top left, top right, left top, left bottom")
...
bpreport = args.bpreport if args.subregion is None else False
skipbp = args.skipbp if args.subregion is None else True
detailed = args.detailed
forcemulti = args.forcemulti if args.subregion is None else True
...
techmap = {
    ...
    **dict(zip(
        ['gas-cc',],
        ['Gas Combined Cycle']*20)),
    **dict(zip(
        ['gas-ct',],
        ['Gas Combustion Turbine']*20)),
    **dict(zip(
        ['geothermal',],
        ['Geothermal']*20)),
    **{f'egs_allkm_{i}':'Geothermal' for i in range(20)},  
...
onlytechs = None
i_plots = ['wind-ons','upv','battery','h2-ct','nuclear','gas-cc-ccs','coal-ccs','gas-cc','gas-ct']
...
for case in tqdm(cases, desc='national capacity'):
    dictin_cap[case] = (reeds.io.read_output(cases[case],'cap_ivrt',r_filter = val_r_subset[case])
        .rename(columns = {'Value':'Capacity (GW)','i':'tech','t':'year'}))
    #reeds.io.read_report(cases[case], 'Capacity (GW)', val2sheet)
    ### Simplify techs
    dictin_cap[case].tech = reedsplots.simplify_techs(dictin_cap[case].tech)
...
for case in tqdm(cases, desc='national generation'):
    dictin_gen[case] = (reeds.io.read_output(cases[case],'gen_ivrt',r_filter = val_r_subset[case])
    ...
    dictin_gen[case].tech = reedsplots.simplify_techs(dictin_gen[case].tech)
...
for case in tqdm(cases, desc='national emissions'):
    dictin_emissions[case] = (
        reeds.io.read_output(cases[case], 'emit_r', valname='ton', r_filter = val_r_subset[case] )
    )
...
aggtechsplot = {
    ...
    'Non-Gas Fossil\n(w/o CCS)': ['o-g-s', 'coal', 'cofire'],
    'Gas\n(w/o CCS)': ['gas-cc', 'gas-ct'],
#     'CDR': ['dac', 'beccs'],
...
if args.four is not None:
    ...
    for slidetitle, data in toplot.items():
        ...
        '''
        ax[0,0].set_ylabel(f'{titles[2]}\n{data["label"]}')
        ax[1,0].set_ylabel(f'{titles[3]}\n{data["label"]}')
        ax[2,0].set_ylabel(f'Difference')
        ax[0,0].set_title(f'{titles[0]}', fontweight='bold', fontsize=14)
        ax[0,1].set_title(f'{titles[1]}', fontweight='bold', fontsize=14)
        ax[0,2].set_title(f'Difference', fontweight='bold', fontsize=14)
        '''

        ax[0,0].set_ylabel(data['label'], y=-0.075)

        ax[0,0].annotate(
            f'{titles[2]}', (-0.5,0.5), xycoords='axes fraction',
            fontsize='x-large', weight='bold', rotation = 'vertical',
            verticalalignment  = 'center', horizontalalignment = 'right')
        ax[1,0].annotate(
            f'{titles[3]}', (-0.5,0.5), xycoords='axes fraction',
            fontsize='x-large', weight='bold', rotation = 'vertical',
            verticalalignment  = 'center', horizontalalignment = 'right')
        ax[0,0].annotate(
            f'{titles[0]}', (0.5,1.25), xycoords='axes fraction',
            fontsize='x-large', weight='bold', horizontalalignment = 'center')
        ax[0,1].annotate(
            f'{titles[1]}', (0.5,1.25), xycoords='axes fraction',
            fontsize='x-large', weight='bold', horizontalalignment = 'center')
        ax[0,0].set_xlim(2017.5, lastyear+2.5)

        ###### Absolute
        alltechs = set()
        x_map = {0:0,1:1,2:0,3:1}
        y_map = {0:0,1:0,2:1,3:1}
        titles_key = {0:[0,2],1:[1,2],2:[0,3],3:[1,3]}
        case_list = list(cases.keys())
        print(case_list)
        for col, case in enumerate(cases):
            ax_i = ax[y_map[col],x_map[col]]
            if case not in data['data']:
                continue
            dfplot = data['data'][case].pivot(index='year', columns=data['columns'], values=data['values'])
            if data['columns'] != 'cost_cat':
                ...
                print(dfplot.sum())
                dfplot = (
                    (dfplot.loc[years[case]])
                )
                net_val = True
            if case == basecase:
                dfbase[slidetitle] = dfplot.copy()
            if col == 1:
                dfbase_1[slidetitle] = dfplot.copy()
            if col == 2:
                dfbase_2[slidetitle] = dfplot.copy()
            if (slidetitle == 'Generation') and (args.subregion is not None):
                net_val = True
            alltechs.update(dfplot.columns)
            
            plots.stackbar(df=dfplot, ax=ax_i, colors=data['colors'], width=yearstep[case]*0.95, net=net_val)           

            ax_i.xaxis.set_major_locator(mpl.ticker.MultipleLocator(10))
            ax_i.xaxis.set_minor_locator(mpl.ticker.MultipleLocator(5))
            if col> 0:
                ax_i.sharey(ax[0,0])
            ax_i.annotate(
                f'{titles[titles_key[col][0]]} & {titles[titles_key[col][1]]}', (0.05,0.95), xycoords='axes fraction',
                fontsize='small', horizontalalignment = 'left', color = '0.5')
        ...
        ###### Difference
        diff_plots_cases = {0:[1,0],1:[3,2],2:[3,0],3:[3,1],4:[2,0]}
        locs =             {0:[0,2],1:[1,2],2:[2,2],3:[2,1],4:[2,0]}
        titles_key =       {0:[2,0,1],1:[3,0,1],2:[0,2,1,3],3:[1,2,3],4:[0,2,3]}
        for col in range(5):
            case = case_list[diff_plots_cases[col][0]]
            if diff_plots_cases[col][1] == 0:
                df_base = dfbase
            elif diff_plots_cases[col][1] == 1:
                df_base = dfbase_1
            elif diff_plots_cases[col][1] == 2:
                df_base = dfbase_2
            ax_i = ax[locs[col][0],locs[col][1]]
            ax_i.xaxis.set_major_locator(mpl.ticker.MultipleLocator(10))
            ax_i.xaxis.set_minor_locator(mpl.ticker.MultipleLocator(5))
            ax_i.axhline(0,c='k',ls='--',lw=0.75)
            ...
            if data['columns'] != 'cost_cat':
                ...
                dfplot = dfplot.loc[years[case]]
            dfplot = dfplot.subtract(df_base[slidetitle], fill_value=0)
            dfplot = dfplot[[c for c in data['colors'] if c in dfplot]].copy()
            alltechs.update(dfplot.columns)
            plots.stackbar(df=dfplot, ax=ax_i, colors=data['colors'], width=yearstep[case]*0.95, net=True)

            if col> 0:
                ax_i.sharey(ax[0,2])
            if col == 2:
                ax_i.annotate(
                    f'{titles[titles_key[col][0]]} & {titles[titles_key[col][1]]} - \n {titles[titles_key[col][2]]} & {titles[titles_key[col][3]]}', (0.05,0.95), xycoords='axes fraction',
                    fontsize='small', horizontalalignment = 'left', color = '0.5')
            else:
                ax_i.annotate(
                    f'{titles[titles_key[col][0]]}:\n{titles[titles_key[col][1]]} - {titles[titles_key[col][2]]}', (0.05,0.95), xycoords='axes fraction',
                    fontsize='small', horizontalalignment = 'left', color = '0.5')
            ax_i.tick_params(axis='y', labelsize=8)

        plots.despine(ax)
        plt.draw()
        plots.shorten_years(ax[2,0])
        for ax_i in [ax[2,0],ax[2,1],ax[2,2]]:
            ax_i.xaxis.set_major_locator(mpl.ticker.MultipleLocator(10))
            ax_i.xaxis.set_minor_locator(mpl.ticker.MultipleLocator(5))
...
#%%### Hodgepodge: Final capacity, final generation, final transmission, NPV, runtime
width = max(11, len(cases)*1.3)
plt.close()
f,ax = plt.subplots(
    2, 5, figsize=(width, SLIDE_HEIGHT), sharex=True,
...
### NPV
col = 3
ax[0,col].set_ylabel('NPV (Bil $)', y=-0.075)
dfplot = pd.concat(
    {case: dictin_npv2[case].groupby(['cost_cat'])['Discounted Cost (Bil $)'].sum() for case in cases},
    axis=1).T.fillna(0)
dfplot = dfplot[[c for c in bokehcostcolors.index if c in dfplot]].copy()

handles['NPV (Bil $)'] = plot_bars_abs_stacked(
...
plt.close()
f,ax = plt.subplots(1, 5, figsize=(15, 0.1))
...
for interzonal_only in [False, True]:
    ...
    if interzonal_only:
        dftrans = (
            dftrans[[c for c in dftrans if c[1].lower()]]
        ).copy()
...
dfba = dfmap['r']
dfba = dfba[dfba['st']=='UT'].copy()  # Only include Utah for plotting
dfstates = dfmap['st']
dfstates = dfstates[dfstates.index.get_level_values('st')=='UT']  # Only include Utah for plotting

#print(dfba,dfstates)
#if args.subregion is not None:
#    dfba
#    dfba = dfba.loc[dfba.index.get_level_values('r').isin(all_rs)]
#    dfstates = dfstates.loc[dfstates.index.get_level_values('st').isin(all_st)]
